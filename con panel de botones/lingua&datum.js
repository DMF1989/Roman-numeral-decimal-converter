window.addEventListener('load', function(){
  ...
  function changelanguaje() {
    ...
    if (lingua.checked) {
      ...
      if (regexDecimal.test(romanField.innerText.replaceAll(' ',''))) {nume.innerText = decEng; boter.innerText = 'Convert to roman number'; response = 'The roman corresponding number is:'} else if (regexRoman.test(romanField.innerText)) {nume.innerText = romEng; boter.innerText = 'Convert to decimal number'; response = 'The decimal arabic corresponding number is: '};
      if (romanField.innerText == '' && !romanField.getAttribute('data-before')) {nume.innerText = romEng; boter.innerText = ''};
      if (romanField.getAttribute('data-before')) {nume.innerText = romEng; boter.innerText = 'Convert to decimal number'}
      ...
      if (regexDecimal.test(romanField.innerText.replaceAll(' ',''))) {nume.innerText = decEsp; boter.innerText = 'Convertir a número romano'; response = 'El número romano correspondiente es:'} else if (regexRoman.test(romanField.innerText)) {nume.innerText = romEsp; boter.innerText = 'Convertir a número decimal'; response = 'El número arábigo decimal correspondiente es: '};
      if (romanField.innerText == '' && !romanField.getAttribute('data-before')) {nume.innerText = romEsp; boter.innerText = ''};
      if (romanField.getAttribute('data-before')) {nume.innerText = romEsp; boter.innerText = 'Convertir a número decimal'}
