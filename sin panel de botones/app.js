...
romanField.addEventListener('input', function(e) {
  ...
  const valor = e.target.value;
    ...
    var num = e.target.value.replace(/[^0-9ivxlcdm]/g, "");
    ...
    if ((/\d/).test(valor)) {base.style.display = 'none'; changelanguaje()} else if (regexRoman.test(valor)) {base.style.display = 'block'; changelanguaje()};
    if (!romanField.value) {base.style.display = 'block'; changelanguaje()};
    del()
})
...
let backspace;
let keyNumber
romanField.onkeydown = function(event) {
  digitSpace = (/^\d\s/).test(romanField.value)
/*   twoDigitSpace = (/^\d\d\s/).test(romanField.value) */
  if (event.key == "Backspace") {backspace = true} else {backspace = false}
  if (event.key == regexDecimal) {keyNumber == true} else {keyNumber = false}
};

function formatNumber(n) {
  // Obtener la posición actual del cursor
  var position = romanField.selectionStart;
  ...
  romanField.value = formatted;
  // Ajustar posición del cursor
  romanField.setSelectionRange(position, position);
  if (digitSpace && backspace && position > 1) {romanField.setSelectionRange(position, position-1)};
  if ((/^\d\d\s/).test(romanField.value) && backspace) {romanField.setSelectionRange(position, position)};
  if ((/^\d\s\d\d\d/).test(romanField.value) && !backspace) {romanField.setSelectionRange(position+1, position+1)};
  return formatted;
}

function del(){
  if(!base.innerText.trim() && !romanField.value) 
    {text.innerHTML = ''; answer.innerHTML = ''; overline = ''; boter.style.display = "none"; nume.style.display = "none" 
  removeElement()}
  else {boter.style.display = "block"; nume.style.display = "block"}
};
...
function result(){
  ...
  text.insertAdjacentElement('afterend', threeLine)}
        ...
        if (regexRoman.test(romanField.value) || base.innerText.trim()) {answer.innerHTML = (Number(romanToDecimal(base.innerText.trim()))*1000 + Number(romanToDecimal(romanField.value))).toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1 ') + '.'; if (answer.classList.contains('margintext')) {answer.classList.remove('margintext')}} 
