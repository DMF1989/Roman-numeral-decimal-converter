...
function changelanguaje() {
  ...
  if (lingua.checked) {
    ...
    if (regexDecimal.test(romanField.value.replaceAll(' ',''))) {nume.innerText = decEng; boter.innerText = 'Convert to roman number'; response = 'The roman corresponding number is: '} 
    if (regexRoman.test(romanField.value) || base.innerText.trim() != false) {nume.innerText = romEng; boter.innerText = 'Convert to decimal number'; response = 'The decimal arabic corresponding number is: '};
    ...
    if (regexDecimal.test(romanField.value.replaceAll(' ',''))) {nume.innerText = decEsp; boter.innerText = 'Convertir a número romano'; response = 'El número romano correspondiente es: '}
    if (regexRoman.test(romanField.value) || base.innerText.trim() != false) {nume.innerText = romEsp; boter.innerText = 'Convertir a número decimal'; response = 'El número arábigo decimal correspondiente es: '};
